...
cells.append(nbformat.v4.new_markdown_cell(
    ...
    "This notebook demonstrates fitting a **four-parameter logistic (4PL / Hill4P)** model "
    "to a simulated dose-response dataset using the `openfit` library.\n\n"
    "## What is the 4PL model?\n\n"
    "The four-parameter logistic (4PL) equation is:\n\n"
    "$$y = \\text{Bottom} + \\frac{\\text{Top} - \\text{Bottom}}"
    "{1 + \\left(\\frac{\\text{EC50}}{x}\\right)^{\\text{HillSlope}}}$$\n\n"
    "| Parameter | Meaning |\n"
    "|-----------|--------|\n"
    ...
    "ELISA measurements exhibit approximately **constant coefficient of variation (CV)**, "
    "meaning the standard deviation scales proportionally with the signal. "
    "When variance is proportional to y², the appropriate weight is **1/y²**, "
    "which gives each point equal influence on the fit in relative (%) terms. "
    "Unweighted least-squares would over-weight high-signal points and under-weight "
    "low-signal points, leading to biased parameter estimates."
))
...
# Cell 3 — Markdown: Generate synthetic data
cells.append(nbformat.v4.new_markdown_cell(
    ...
    "Multiplicative noise with ~15% CV is added to mimic a typical ELISA plate, "
    "where measurement variability is proportional to the signal level."
))
...
# Cell 5 — Markdown: Fit the model
cells.append(nbformat.v4.new_markdown_cell(
    "## Fit the 4PL model\n\n"
    "We pass the model name `\"hill4p\"`, the concentration array `x`, "
    "and the response array `y` to `Fit`, then call `.run()` to perform "
    "nonlinear least-squares optimization.\n\n"
    "**Weight choice — `weights=\"1/y2\"`:**  \n"
    "Because ELISA CV is approximately constant (variance ∝ y²), "
    "the 1/y² weight scheme is the statistically correct choice. "
    "Each residual is divided by the measured response, so points at all "
    "signal levels contribute equally in percentage terms."
))

# Cell 6 — Code: Run the fit
cells.append(nbformat.v4.new_code_cell(
...
cells.append(nbformat.v4.new_markdown_cell(
    ...
    "A good fit shows: recovered parameters close to truth, narrow CIs, "
    "R² near 1, and standardized residuals randomly scattered between −2 and +2."
))

# Cell 8 — Code: Access individual results
cells.append(nbformat.v4.new_code_cell(
...
# Cell 9 — Markdown: Plot the fit
cells.append(nbformat.v4.new_markdown_cell(
    "## Plot the fit\n\n"
    "Two panels are shown side by side:\n\n"
    "- **Left:** The fitted 4PL curve overlaid on the raw data, "
    "with a vertical dashed line marking the EC50.\n"
    "- **Right:** Standardized residuals vs. concentration on a log scale. "
    "Residuals should be randomly scattered around zero with no systematic curvature "
    "(which would indicate model misspecification) and most points within ±2."
))
...
# Cell 11 — Markdown: Save report
cells.append(nbformat.v4.new_markdown_cell(
    "## Save a report and reproducibility manifest\n\n"
    "openfit can export two artifacts:\n\n"
    "1. **HTML report** — a self-contained file with the fit plot, "
    "parameter table, diagnostic statistics, and the model equation.\n"
    "2. **FitSpec JSON** — a lightweight reproducibility manifest that captures "
    "the model name, weight scheme, fitted parameters, software versions, and a "
    "hash of the input data. Anyone with this spec can reconstruct the exact fit "
    "without re-running the notebook."
))

# Cell 12 — Code: Save report and FitSpec
cells.append(nbformat.v4.new_code_cell(
...
cells.append(nbformat.v4.new_markdown_cell(
    "## Summary\n\n"
    "In this notebook we:\n\n"
    "1. **Simulated** a realistic ELISA-style dose-response dataset using a known 4PL curve "
    "with 15% CV multiplicative noise.\n"
    "2. **Fitted** the `hill4p` model with `openfit`, using 1/y² weighting appropriate "
    "for constant-CV data.\n"
    "3. **Inspected** the results — recovered parameters, standard errors, 95% confidence "
    "intervals, R², and AICc.\n"
    ...
    "The `FitSpec` JSON contains everything needed to reproduce this exact result — "
    "model, parameters, weight scheme, data hash, and software versions. "
    "See `05_reproducibility.ipynb` for a full reproducibility demo.\n\n"
    "**Next notebooks:**\n"
...
# Ensure all code cells have proper metadata
for cell in nb.cells:
    if cell.cell_type == "code":
        cell.execution_count = None
        cell.outputs = []

output_path = "/sessions/happy-wonderful-goldberg/mnt/openfit/notebooks/01_dose_response.ipynb"
...
with open(output_path) as f:
    data = json.load(f)
print(f"nbformat version: {data['nbformat']}.{data['nbformat_minor']}")
print(f"Number of cells: {len(data['cells'])}")
for i, cell in enumerate(data['cells'], 1):
    print(f"  Cell {i:2d}: {cell['cell_type']}")
