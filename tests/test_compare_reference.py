...
def two_site_binding(
    x: np.ndarray, Bmax: float, Kd: float, Bmax2: float, Kd2: float
) -> np.ndarray:
...
def initial_guess_one_site(x: np.ndarray, y: np.ndarray) -> dict[str, float]:
    ...
    y_sorted = y[sort_idx]
    if len(x_sorted) >= 2:
        Kd = float(np.interp(half_max, y_sorted, x_sorted))
    else:
        Kd = float(x[0])
...
def test_f_test_formula_matches_manual_calculation():
    ...
    f_stat_manual = (
        (rss_simple - rss_complex) / df_numerator
    ) / (rss_complex / df_denominator)
    ...
    assert comparison.f_test.df_numerator == df_numerator, (
        f"df_numerator: expected {df_numerator}, "
        f"got {comparison.f_test.df_numerator}"
    )
    assert comparison.f_test.df_denominator == df_denominator, (
        f"df_denominator: expected {df_denominator}, "
        f"got {comparison.f_test.df_denominator}"
    )
    
    # Verify F-statistic (tolerance 1e-6 for numerical precision)
    assert np.isclose(
        comparison.f_test.f_statistic, f_stat_manual, rtol=1e-6
    ), (
    ...
    assert np.isclose(
        comparison.f_test.p_value, p_value_manual, rtol=1e-6
    ), (
        f"p-value mismatch:\n"
    ...
    assert np.isclose(
        comparison.f_test.rss_simpler, rss_simple, rtol=1e-10
    ), f"RSS_simpler mismatch: {comparison.f_test.rss_simpler} vs {rss_simple}"
    
    assert np.isclose(
        comparison.f_test.rss_complex, rss_complex, rtol=1e-10
    ), f"RSS_complex mismatch: {comparison.f_test.rss_complex} vs {rss_complex}"


def test_f_test_prefers_complex_model_for_true_two_site_data():
    ...
    assert comparison.f_test.preferred_model == "complex", (
        f"Expected 'complex' model to be preferred, got "
        f"'{comparison.f_test.preferred_model}'"
    )


def test_f_test_does_not_prefer_complex_for_one_site_data():
...
def test_f_test_with_low_noise():
...
def test_f_test_components_consistency():
    ...
    f_recomputed = (
        (f_test.rss_simpler - f_test.rss_complex) / f_test.df_numerator
    ...
    p_recomputed = float(
        stats.f.sf(f_test.f_statistic, f_test.df_numerator, f_test.df_denominator)
    )
