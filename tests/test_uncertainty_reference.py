...
def test_profile_ci_asymmetric_for_ec50() -> None:
    ...
    assert ec50_lo > 0 and ec50_hi > 0, f"EC50 CI must be positive: [{ec50_lo:.2e}, {ec50_hi:.2e}]"
...
def test_profile_ci_asymmetric_for_hillslope() -> None:
...
def test_profile_ci_contains_true_params() -> None:
...
def test_profile_ci_vs_asymptotic_ci() -> None:
    ...
    aci = asymptotic_ci(result.params, result.se, result.n_obs, result.n_params, confidence=0.95)
    pci = profile_likelihood_ci(result, confidence=0.95, n_steps=50)
...
def test_profile_ci_unimodal_no_warnings() -> None:
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        unimodal_warnings = [
            str(warning.message) for warning in w if "non-unimodal" in str(warning.message).lower()
        ]
    ...
    for param_name in result.params:
        assert pci.unimodal[param_name], f"Profile for {param_name} should be unimodal"
        assert pci.converged[param_name], f"Profile CI for {param_name} should converge"
...
def test_profile_ci_synthetic_asymmetric_data() -> None:
