...
def _biphasic_dose_response(x, bottom1, top1, ec50_1, slope1, 
                             bottom2, top2, ec50_2, slope2, fraction):
...
def test_rout_figure2_biphasic_with_outliers():
    ...
    assert fp_rate <= 0.05, (
        ...
        f"FDR should be controlled at Q=1%."
    )
...
def test_rout_table1_q_parameter_monotonicity():
...
def test_rout_table1_fdr_control_on_clean_data():
...
def test_rout_30_percent_scan_rule():
...
def test_rout_sequential_fdr_procedure():
...
def test_rout_rsdr_calculation():
...
def test_rout_handles_few_outliers():
    ...
    assert result.n_outliers >= 1, (
        f"ROUT failed to detect any of 2 injected outliers. "
        f"Expected at least 1 detection."
    )


def test_rout_handles_many_outliers():
...
def test_rout_rejects_invalid_q():
...
def test_rout_performance_matches_paper_claims():
    ...
    false_positives = sum(
        ...
        if i not in outlier_idx and result.outlier_mask[i]
    )
